	...
	$.mbExtruder= {
		...
		buildMbExtruder: function(options){
			return this.each (function (){
                ...
                var h = $(window).height();
				var c= $("<div/>").addClass("extruder-content").css({overflow:"auto", width:extW,height:h});
                ...
                var t = 0;
                if(extruder.metadata().top) t=extruder.metadata().top;
				extruderStyle =
								this.options.position=="top" ?
				{position:position,top:t,left:"50%",marginLeft:-this.options.width/2,width:this.options.width}:
								this.options.position=="bottom" ?
				{position:position,bottom:0,left:"50%",marginLeft:-this.options.width/2,width:this.options.width}:
								this.options.position=="left" ?
				{position:position,top:t,left:0,width:1}:
				{position:position,top:t,right:0,width:1};
				extruder.css(extruderStyle);
				if(!isIE) extruder.css({opacity:this.options.extruderOpacity});
                if(this.options.position=="left"){
                    extruder.css({'border-right':"4px solid #eb1b22"});
                }
                if(this.options.position=="right"){
                    extruder.css({'border-left':"4px solid #eb1b22"});
                }
...
*/
		...
		openMbExtruder:function(c){
            $('.block-content').hide();
            $('.block-footer').hide();
            $('.extruder-container .block-content').show();
            $('.extruder-container .block-footer').show();
            ...
            /*if(position == 'right'){
                extruder.find(".extruder-container").css({'border-right': '5px solid #eb1b22'});
            }*/

            extruder.find('.flap').addClass('active');
            extruder.find('.flap').css('top','65px');
            if (extruder.metadata().title_back){
                var flapLabel = extruder.find(".flapLabel");
                flapLabel.text(extruder.metadata().title_back);
            }
		...
		closeMbExtruder:function(){
            ...
            extruder.find('.flap').removeClass('active');
            extruder.find('.flap').css('top','100px');
            if (extruder.metadata().title_back){
                var flapLabel = extruder.find(".flapLabel");
                flapLabel.text(extruder.metadata().title);
            }
            $('.block-content').show();
            $('.block-footer').show();
		}
